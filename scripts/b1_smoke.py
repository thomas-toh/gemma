...
Proves the three things the B1 brain adapter (spec/20) will rely on:
...
--------------------------------------------------------------------
1. console.anthropic.com → create a DEDICATED key for this project (name it "gemma")
...
2. Store the key in the OS credential store (spec/50 rule 10) — never setx, never a
   repo file. Same command on Windows (Credential Manager) and macOS (Keychain):
     pip install keyring
     python -c "import keyring; keyring.set_password('gemma','anthropic', input('key: ').strip())"
   (input() keeps the key out of your shell history.)
3. Install the SDK:  pip install anthropic
4. Run:              python scripts/b1_smoke.py
...
If the model id has aged out (error mentions the model), set GEMMA_SMOKE_MODEL to a
...
DONE-WHEN (record in STATE.md Track C):
  ...
  [ ] key lives in Credential Manager under service "gemma" (not setx, not any file)
...
def get_key() -> str:
    ...
    try:
        ...
        if key:
            print("  key source: OS credential store (service 'gemma')")
            return key
    except ImportError:
        pass
...
def main() -> None:
    ...
    try:
        ...
        fail(f"Model '{MODEL}' not found — set GEMMA_SMOKE_MODEL to a current id.")
