...
DAEMON_MODEL = os.environ.get("GEMMA_MODEL", "claude-opus-4-8")
...
class Orchestrator:
    ...
    def _run_async(self, coro):
        ...
        if self._loop is None:
            ...
            threading.Thread(target=self._loop.run_forever, name="gemma-model",
                             daemon=True).start()
...
def _selfcheck() -> None:
    ...
    # never be fed to the model — the seam the adversarial review flagged (_pressed has two
    ...
    with _tf.TemporaryDirectory() as _tmp:
        ...
        _st.set("profile_name", "Thomas Smith")
        _st.set("profile_called", "Tom")
        _p = _o._persona()
        assert _p.startswith(DEFAULT_SYSTEM), "the voice still opens the prompt"
        assert "speaking to is Thomas Smith. Address them as Tom." in _p, _p
        ...
        _st.set("profile_called", "thomas smith")
        assert _o._persona().count("Thomas Smith") == 1, _o._persona()
...
def main() -> None:
    ...
    ap.add_argument("--model", default=DAEMON_MODEL,
                    help=f"model id (default {DAEMON_MODEL}; env GEMMA_MODEL)")
