...
The table is spec/schemas/word_replacements.json (hard rule 3 — the schema is the truth, this
...
from .config import load_schemas
...
def _selfcheck() -> None:
    ...
    assert apply("i love gemma", tbl) == "i love Gemma"
    assert apply("GEMMA rocks", tbl) == "Gemma rocks"
    assert apply("the api call", tbl) == "the API call"
    # No partial-word matches: 'gemma' inside 'gemmata', 'api' inside 'therapist'.
    assert apply("gemmata", tbl) == "gemmata"
    ...
    # The shipped schema loads, is well-formed, and fixes the app's own name.
    shipped = load_schemas()["word_replacements"]["replacements"]
    assert isinstance(shipped, list)
    assert apply("gemma", shipped) == "Gemma"
