"""The router (spec/20 §Routing) — resolve a ROLE to the brain that serves it, from the user's
...
settings. Three roles today:
  ...
  assistant          the answer brain            <- settings `primary`
...
def build_for_role(role: str):
    ...
    providers.build_brain, so B1 for the anthropic wire and B2 for the openai wire — the router
...
def _selfcheck() -> None:
    # ponytail: runnable check of the resolution logic — no network, no adapter built (build_brain
