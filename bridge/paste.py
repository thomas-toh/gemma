...
later seam (D10): there the functions no-op with a warning, exactly as the hotkeys do.

    python -m bridge.paste "hello"     # set the clipboard + paste into the focused window
    python -m bridge.paste --selfcheck # no keystrokes: clipboard round-trip + the encoders
"""
...
log = logging.getLogger("gemma.paste")
...
def _selfcheck() -> None:
    ...
    if sys.platform != "win32":
        # Every entry point must degrade, never raise, off Windows (macOS until D10).
    ...
    sentinel = "gemma-paste-selfcheck-é—你好"   # accents, em-dash, CJK
...
def main() -> None:
    ...
    setup_logging()
    ap = argparse.ArgumentParser(description="Gemma dictation delivery — clipboard + Ctrl+V (D12)")
