...
Cross-platform (spec/00 D10, macOS is a full peer): sounddevice (PortAudio) and
...
Run:
    python -m bridge.audio.wake              # live: speak the wake phrase
    python -m bridge.audio.wake --selfcheck  # no mic needed: verify buffer discipline
"""
...
def main() -> None:
    setup_logging()
    ap = argparse.ArgumentParser(description="Gemma wake-word listener (Track G step 2)")
