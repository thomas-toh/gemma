...
def setup_logging(level: int = logging.INFO, to_file: bool = True) -> None:
    ...
    if to_file:
        try:
            ...
            logging.getLogger("gemma.log").warning("file logging off (%s)", exc)
    logging.basicConfig(
        level=level,
...
def _selfcheck() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        LOG_DIR, LOG_FILE = Path(tmp), Path(tmp) / "gemma.log"
        reset()
        setup_logging()
        logging.getLogger("gemma.selfcheck").info("hello from the selfcheck")
