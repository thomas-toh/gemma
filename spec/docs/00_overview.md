...
Gemma is a *UI-first desk assistant*, with a Python frontend (Qt/QML) and backend, and supports cloud models (via API) or local models (inference through Ollama, llama.cpp or LM Studio). It currently works for Windows (Mac TBC).

[[COWORK - I WANT A NEW MILESTONE CHART BASED ON THE CURRENT ROADMAP.md]]

## Repo architecture

**Repo chart**

[[COWORK - POPULATE THIS WITH THE CHART FROM CLAUDE.MD.]]

**Naming scheme**

This project has many moving parts, so for ease of reference I set out the terminology used in this documentation and in the codebase: 

| Letter | Element | Description |
|--------|---------|-------------|
| **G** | Bridge | This is the coordinator that bridges the backend's work to the frontend. `backend/orchestrator.py` is the primary load-bearing element of this bridge, for it coordinates between the receipt of messages from the input source, feeds status messages to the frontend, and manages the router's calls at the backend. |
| **M** | Model | These are the LLM models added via API or locally via Ollama, LM Studio or llama.cpp. |
| **T** | Tools | These are the tools designed to run actions on the PC. For safety we have divided this into 3 tiers, each gated by functionality and pervasiveness to the system. |
| **P** | Teleprompter | This is the separate Qt/QML frontend process which receives the status feed from `backend/orchestrator.py` and displays it to the user. The teleprompter would expose a limited set of controls, such as copy/save for responses, play/pause for media and audio, open/share for files and emails, and others to be constructed. |

## Project architecture

The Python backend daemon handles the tool calls and LLM, and broadcasts over a local server to the Qt/QML frontend (conveniently called the Teleprompter). The design methodology was to preserve the function of the backend tool/LLM calls even if the frontend crashed, but the necessity of preserving this capability is being investigated due to a strong sense of YAGNI. The app is controllable (currently) via two "hotkeys": (1) *Ask*, which opens an assistant, transcribes via STT, routes it, then broadcasts the result to the teleprompter; and (2) *Dictate*, which is a simple dictation engine that transcribes via STT, cleans up (optional), and pastes at the current cursor caret. 

So: 

> **Target architecture — router v2 is not yet built; the classify-and-dispatch below is subject to change.** In today's code the router only resolves a role to a model, and the model itself emits the tool calls.

```mermaid
flowchart LR
    U["ask / dictate hotkey"] -- "speech → STT" --> G["Bridge (orchestrator)"]
    G --> R["router (v2) — classifies the request"]
    R -- "assistant call" --> M["Model — cloud API or local LLM"]
    R -- "tool call" --> T["Tools (tiered, settings-gated)"]
    G -- "status feed (one-way)" --> P["Teleprompter + audio out"]
```

## Frontend

The frontend is ... [[CLAUDE COWORK - POPULATE THIS WITH A SHORT EXPLANATION OF THE FRONTEND. EXPLAIN GENERAL PLATFORMING DECISIONS.]]

## Backend

The backend is ... [[CLAUDE COWORK - POPULATE THIS WITH A SHORT EXPLANATION OF THE BACKEND. EXPLAIN THE HIGH LEVEL STRUCTURAL FLOW, A TURN OR TOOL CALL, ETC. MORE DETAIL IN THE LATER SECTIONS.]]
...
| **Teleprompter** (P) — overlay, separate process on the status feed | [40_interaction](40_interaction.md) § Visual output | `teleprompter/` | v0 pre-M0-run (D13 [deleted]/D19) · dictation states at D2 |
| Orchestrator (state machine) | [40_interaction](40_interaction.md) | `bridge/orchestrator.py` | M0 (build step 6) |
| Brain adapters | [20_models](20_models.md) | `bridge/brains/` | B1 + B2 built (D30) · B3 at M4 |
| Tool registry + executor | [30_tools](30_tools.md) + [schemas/tools.json](schemas/tools.json) | `bridge/tools/` | M1 |
...
cleaned → compare brain answers) decides default-on or delete. D11 [deleted]'s bounds apply to
...
Amends D11 [deleted] as noted there.
...
halves of D12 (parking → cancellation), D13 [deleted] ("a second headset made of pixels"), and D16
...
**Contract P** (`spec/schemas/status.json`). Architecture (D13 [deleted] restated as a contract):
...
"listen for me" decision is **settled here** rather than left owed. (d) D11 [deleted]'s feedback budget is
...
acceptance criteria are **D11 [deleted] (2026-07-12) — written for the eyes-free headset era**, and never
...
- **`first_word` (< 4 s B1 / < 5 s B2) was a latency gate that is really a reply-length cap.**
  Under generate-then-play (D11 [deleted]) the first spoken word waits for the *whole* reply to be
  ...
  with sentence-streamed TTS (parked, D11 [deleted]) — reopen that with the measured number, not on feel.
...
- **The numbers lived in four places** (spec/40 prose · `Overlay.qml` · the orchestrator's
  ...
  the pre-D11 [deleted] values). Consolidated into **`spec/schemas/targets.json`** (executable truth, hard
...
island's austerity — spawned on demand from the tray (D13 [deleted], zero idle cost) and reading
...
- **Consent is stated twice: before and during.** A card lists the tools it enables and what each
  ...
  reserved tool-activity indicator (D11 [deleted]) finally getting a renderer, plus a new Contract P message.
...
not an architecture one. Merging would have paid for it with the three things D13 [deleted] bought.
...
- **The tie, and its exception.** Whichever process exits **cleanly** (code 0) stops the other; a
  ...
  console are each one door out of the whole app, while a crash still behaves exactly as D13 [deleted]/D19
...
- **Nothing is amended.** This was expected to amend D13 [deleted]/D19's two-process isolation rationale and
