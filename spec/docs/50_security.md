...
**Status: BINDING (design constants, not preferences)** · Last reconciled: 2026-08-03 · Rationale: docs/01 §7, docs/02 §7
...
4. **Truthful signalling.** The overlay's listening indicator must reflect actual pipeline
   state; `listening` ⇔ audio is being captured/processed. No dark listening, ever. The wake word is a trained keyword model (openWakeWord), never an LLM and never continuous transcription — capture starts only on a local phrase match, so nothing reaches STT before the trigger.
