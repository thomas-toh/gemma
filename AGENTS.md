# Gemma assistant app instructions for LLMs (AGENTS.md)
...
4. **Rule 4 - Most fast, strike hard:** User's codebase is expected to shift quickly in prototyping phase. When coding:
...
7. **Rule 8 - Draft discipline:** All agentically updated files - `AGENTS.md`, `STATE.md`, and `ROADMAP.md`, should be kept thin. Be careful where you put information. Decisions which just reflect certain choices but do not constitute project-important decisions belong in NOTES. If not, it belongs in `spec/`.
...
```
AGENTS.md          ← you are here; keep THIN (index + rules only, never the spec itself)
...
I am trying to ideate a project for a product which would allow the user to communicate with AI tools and augment AI functionality through a headset (LLM would preferably be a local LLM).  I have little experience with product design and development, limited experience with code, but a lot of ideas and willingness to spend some money.  I need support in this project, including where it would not likely be profitable.  Don't need to be particularly encouraging re enterprise or to invent product moats - the idea is not necessarily to bring something to market, but to bring an idea to market.  Think of it as a tinkering project of a sci-fi obsessed tinkerer who finally has access to adult money, modern tech and AI assistants.
