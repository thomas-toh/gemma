...
Reads the sprite kit (`teleprompter/gem/gem-sprites.json`, kit **v3**: palette-indexed 26×26
...
export overwrites it) and paints a frame to a QImage / QIcon. Two consumers:
  ...
  - the settings window, via `GemImageProvider` (`image://gem/<state>/<clip>/<frame>`), driven
    by `QmlGem` so QML never reimplements the kit's timing.

*(The tray is NOT a Gem surface — it draws a mic-level ring of its own, `tray.py`.)*
...
single frame, so without that script Gem is frozen — hence `GemPlayer` here (a port of the kit's
own `gem_sprites.py::GemPlayer`, minus Pillow).

*(The kit is Design's 26px build — the same artwork as their 32px one, cropped to a tighter cell, so
Gem is 54% of the cell's width instead of 44% and renders 1.23× at a given box. Nothing here reads
...
def app_icon(px: int = 256) -> QIcon:
    """The app / Windows-taskbar icon: the resting Gem on a rounded near-black chip. The chip is
...
class GemPlayer:
    """The kit's script, run for you — a Qt-free port of `gem_sprites.py::GemPlayer`.
...
class QmlGem(QObject):
    """`GemPlayer` on a QTimer, exposed to QML as one bindable `source` URL.
    ...
    and not in QML because Gem now shows on two surfaces — the island and the settings bar — off
    one player: two QML Bindings writing `gemState` would fight, and neither window is reliably
    ...
    def _follow(self) -> None:
        ...
        typewriter (`revealing`), not the daemon's stream, so Gem mimes what is on screen: the
        ...
        if m.state == "listening":
            ...
            # The laptop, for anything where the machine is chewing on it (Thomas): the brain
    ...
    gemState = Property(str, _get_state, _set_state, notify=changed)
...
def _parse_id(image_id: str) -> tuple[str, str, int]:
    ...
    the resting Gem, so a QML typo shows the character rather than a broken-image glyph."""
...
class GemImageProvider(QQuickImageProvider):
    """Serves `image://gem/<state>/<clip>/<frame>` to QML in the ISLAND palette (every app surface
    Gem shows on is dark). Registered once on the QML engine; `QmlGem` steps the URL.
...
def _selfcheck() -> None:
    ...
    assert lit and max(lit) - min(lit) > 64 * 0.5, \
        f"Gem does not fill the app-icon chip (spans {max(lit) - min(lit)}px of 64)"
    ...
    p = GemPlayer("idle", random.Random(7))
    played, seen = [], {p.clip}
    ...
    q = QmlGem()
    assert q.source.startswith("image://gem/idle/"), q.source
    q.gemState = "listening"
    assert q.gemState == "listening" and "/listening/" in q.source
    ...
    # `done` and `error` enter on a HOLD (sparkle, fail), which freezes by design. QmlGem must
    # release an enter-hold once it has played, or Gem stares at the sparkle forever — the kit
    ...
    print(f"gem selfcheck OK: kit v{_data()['version']}, {len(ss)} states / "
          f"{sum(len(clips(s)) for s in ss)} clips at {cell()}px, palettes from the JSON, "
          f"script-driven player, image://gem provider (bad id -> idle)")
