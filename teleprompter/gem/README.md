...
Gem is the character that represents Gemma while she works: a pixel sprite
derived from the Gemma mark itself (the 3/4 disc with the hard square corner).
...
> `teleprompter/gem.py` is this app's renderer (Qt, no Pillow) and ports the
...
kit = GemSprites("assets/gem/gem-sprites.json")
...
1. Replace `gem-sprites.json` and all three PNGs.
...
the app move together. Never hand-edit `gem-sprites.json` or repaint the PNGs;
